jQuery(document).ready(function ($){
  ...
  $.get(urAllocation, function (data){
    ...
    stateSelect.change(function (){
      ...
      if(filteredUni.length != 0) { //If there is at least one university to select
        universitySelect.append(genericOption);
      }

      filteredUni.forEach(function (allocation){
        universitySelect.append(
        optionBase.replace(new RegExp(placeholder,'g'),allocation[universityField])
        )
      })

      //If there are not segmented universities, do not show "University" field
      if(filteredUni.length == 0) {
        universitySelect.append(
        optionBase.replace(placeholder,'other')
        .replace(placeholder,"Todas las Universidades")
        )
        ...
        universitySelect.append(
        optionBase.replace(placeholder,'other')
        .replace(placeholder,"Otra Universidad")
        )
